// components/tourandact/TourList.jsx
"use client";
...
const FALLBACK_IMG = "/fallbacks/tour-card.jpg"; // letakkan file ini di public/fallbacks/
...
export default function TourList() {
    ...
    return (
      <section className="container mx-auto px-4 py-12 md:py-16">
        <h2 className="font-display text-3xl md:text-4xl text-slate-900">
          <span className="border-b-4 border-yellow-500 pb-1">Strolling</span>{" "}
          Around Kotagede
        </h2>
        <div className="mt-6 text-center py-12">Loading tours...</div>
      </section>
    );
  }
  ...
  return (
    <section className="container mx-auto px-4 py-12 md:py-16">
      ...
      >
        {tours.map((t) => (
          <TourCard key={t.slug ?? t.title} t={t} />
        ))}
      </motion.div>
...
function TourCard({ t }) {
  ...
  const imgSrc = t.image || t.hero || FALLBACK_IMG;
      ...
      >
        <div className="relative aspect-[4/5]">
          ...
          <span className="absolute left-3 top-3 z-10 rounded-full bg-yellow-500/95 px-2.5 py-1 text-[10px] font-semibold tracking-wide text-white uppercase">
            {(Array.isArray(t.category) && t.category[0]) || t.tag || "Culture"}
          </span>

          <div className="absolute inset-x-0 bottom-0 z-10 p-4 md:p-5">
            <h3 className="font-display text-white text-xl leading-snug drop-shadow">
              {t.title}
            </h3>
            ...
            {t.price && (
              <p className="mt-2 inline-flex items-center gap-1.5 text-[12px] text-white/90">
                <span className="font-medium">From</span> {t.price}
              </p>
            )}
