// components/tourandact/SimilarExperiencesCarousel.jsx
"use client";
...
export default function SimilarExperiencesCarousel() {
  ...
  useEffect(() => {
    async function fetchTours() {
      try {
        ...
        // pilih yang berkategori workshop, kalau kosong ambil 10 pertama
        const filtered =
          tours.filter((t) => t.category?.includes("workshop")).slice(0, 10) ||
          tours.slice(0, 10);
        setItems(filtered);
      } catch (error) {
  ...
  return (
    <section className="container mx-auto px-4 py-14 md:py-20">
      <div className="text-center mb-8">
        ...
        >
          Discover more cultural activities and workshops in Kotagede
        </motion.p>
      </div>
        ...
        >
          ...
          {items.map((t, idx) => (
            <motion.article
              ...
              viewport={{ once: true }}
              className="shrink-0 snap-start w-[85%] sm:w-[55%] md:w-[40%] lg:w-[30%] bg-white border border-slate-200 rounded-xl overflow-hidden shadow-[0_20px_50px_-25px_rgba(0,0,0,0.2)] transition-transform hover:-translate-y-1"
            >
